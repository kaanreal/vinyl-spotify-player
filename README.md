...
A production-ready Raspberry Pi project that transforms your Pi into a Spotify Connect speaker with a vintage vinyl record player interface. Features a 480x480 circular display showing album art, track metadata, and playback progress, controlled by a mechanical tonearm that triggers play/pause.

## Features

- **Spotify Connect**: Appears as "Vinyl Player" in Spotify Connect on any device
- **Raspotify/librespot**: Hardware-level Spotify playback (no browser, no web player)
- **Circular Display**: 480x480 display with circular album art, track info, and progress bar
- **Tonearm Control**: Physical GPIO-based tonearm triggers play/pause
- **Headless Operation**: No desktop environment required (Raspberry Pi OS Lite)
- **Auto-start**: Boots directly into working state
- **MPRIS Integration**: Metadata via DBus (no Spotify Web API required)
- **Album Art Caching**: Efficient local caching of album artwork
- **Production Ready**: Systemd services, error handling, graceful shutdown

## Hardware Requirements

- Raspberry Pi 4 (or Pi 3B+)
- 480x480 round LCD display (SPI or HDMI)
- Mechanical switch for tonearm (connects to GPIO pin 17)
